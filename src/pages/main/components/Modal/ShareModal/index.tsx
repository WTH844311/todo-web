...
import {Button, message, Modal as AntdModal, Switch} from "antd";
import getIcon from "../../../../../common/icons";
import {shareLink_prefix} from "../../../../../common/config";
import {ModalProps} from "../index";
import {inject, observer} from "mobx-react";

const Index: FC<Omit<ModalProps, 'selected_task'>> = ({selected_list, data, state}) => {
  const {listAction, users = []} = data || {}
  const {
    shareOptionModalVisible,
    changeShareOptionModal,
    changeShareAccessManagement,
    shareAccessManagementModalVisible
  } = state || {}
  ...
  return (
    <>
      <AntdModal
        ...
        width={350}
        centered
        visible={shareOptionModalVisible}
        onOk={changeShareOptionModal}
        onCancel={changeShareOptionModal}
        footer={selected_list.sharing_status !== 'NotShare' && [
          selected_list.owner_id === user_id ? (
            <div
              key='button'
              onClick={changeShareAccessManagement}
              style={{
                display: 'flex',
            ...
            <div className='SOModal-members'>
              ...
              {selected_list.members?.map((member, index) => (
                <div key={index} className='SOModal-memberItem'>
                  ...
                  {selected_list.owner_id === user_id && (
                    <div className='SOModal-memberDelete'
                         onClick={() => listAction.removeMember(member, selected_list)}>
          ...
          {selected_list.sharing_status === 'Open' && (
            <div className='CopySharingLink'>
              ...
              <Button onClick={() => {
                ...
                // @ts-ignore
                shareLinkInput && shareLinkInput.select()
      ...
      <AntdModal
        ...
        width={350}
        centered
        visible={shareAccessManagementModalVisible}
        onOk={changeShareAccessManagement}
        onCancel={changeShareAccessManagement}
        footer={null}
      >
        <div className='SAMModal-limitAccess'>
          <div className='limitAccess-around'>
            <span>将访问权限限制于当前成员</span>
            <Switch checkedChildren="开" unCheckedChildren="关" defaultChecked={selected_list.sharing_status === 'Limit'}
                    onClick={() => listAction.limitShare(selected_list)}/>
          </div>
        ...
        <div className='SAMModal-link'>
          <span>邀请链接</span>
          {selected_list.sharing_status === 'Open' &&
          <p style={{userSelect: 'all'}}>{`${shareLink_prefix}sharing/${selected_list.invitation_token}`}</p>}
        </div>
